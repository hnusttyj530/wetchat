# -*- coding: utf-8 -*-
"""
Created on Fri May  3 10:04:07 2019
...
def get_friends_info():
    ...
    friends_info = dict(
        #国家
        country = get_key_info(friends,"Country"),
        # 省份
...
def analysisCountry():
    friends_info = get_friends_info()
    df = pd.DataFrame(friends_info)
    country_count = df.groupby('country', as_index=True)['country'].count().sort_values()
    temp = list(map(lambda x: x if x != '' else '未知',list(country_count.index)))
    # 画图
    page = Page()
    style = Style(width=1100, height=600)
    style_middle = Style(width=900, height=500)
    attr, value = temp, list(country_count)
    chart1 = Map('好友分布(世界地图)', **style.init_style)
    chart1.add('', attr, value,maptype="world", is_label_show=True, is_visualmap=True, visual_text_color='#000')
    page.add(chart1)
    chart2 = Bar('好友分布柱状图', **style_middle.init_style)
    chart2.add('', attr, value, is_stack=True, is_convert=True,
               label_pos='inside', is_legend_show=True, is_label_show=True)
    page.add(chart2)
    page.render('analysisCountry.html')
